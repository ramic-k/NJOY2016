...
phonon expansion method. Supports:
  ...
  - Generalized coherent inelastic (ncoh_inel=1, FLASSH-style)
      ...
      * Per-species anisotropic Debye-Waller tensor
      * Powder-averaged over random Q directions
  - Incoherent elastic
...
----------------------------------------------------------
    ...
    mesh_nx/ny/nz     BZ mesh points per axis for phonopy (default 30 30 30)
...
WL2EKIN = 0.081804209605330899  # lambda(Å)² × E(eV) = ℏ²/(2m_n), NCrystal value

# Constants for coherent one-phonon inelastic (Phase 1)
THZ_TO_EV = 4.13566733e-3       # 1 THz = 4.13567 meV
PHONOPY_FREQ_FACTOR = 15.633302300230191  # phonopy internal → THz conversion
...
# ============================================================================
# Coherent one-phonon inelastic helpers
# ============================================================================

def _random_sphere_directions(ndir, seed=42):
    """Generate ndir quasi-uniform directions on the unit sphere.

    Uses a Fibonacci lattice for reproducible, low-discrepancy sampling.
    Returns array of shape (ndir, 3).
    """
    golden_ratio = (1.0 + np.sqrt(5.0)) / 2.0
    indices = np.arange(ndir)
    theta = np.arccos(1.0 - 2.0 * (indices + 0.5) / ndir)
    phi = 2.0 * pi * indices / golden_ratio
...
def _splat_to_beta_grid(beta_grid, beta_val, weight, nbeta):
    """Distribute a delta-function contribution at beta_val onto the beta grid.

    Uses linear interpolation (hat function) to distribute weight to the
    two nearest grid points. Returns array of shape (nbeta,).
    """
    contrib = np.zeros(nbeta)
    if beta_val < 0 or beta_val > beta_grid[-1]:
        return contrib
    # Find bracketing indices
    idx = np.searchsorted(beta_grid, beta_val) - 1
    if idx < 0:
        idx = 0
    if idx >= nbeta - 1:
        contrib[nbeta - 1] += weight
        return contrib
    db = beta_grid[idx + 1] - beta_grid[idx]
    if db <= 0:
        contrib[idx] += weight
        return contrib
    frac = (beta_val - beta_grid[idx]) / db
    contrib[idx] += weight * (1.0 - frac)
    contrib[idx + 1] += weight * frac
    return contrib


def _batch_splat(beta_grid, beta_vals, weights, output, nbeta):
    """Batch version of _splat_to_beta_grid: add multiple contributions at once.

    Distributes delta-function contributions at beta_vals onto the beta grid
    using linear interpolation. Modifies output in-place.
    """
    mask = np.abs(weights) > 1e-30
    if not np.any(mask):
        return
    bv = beta_vals[mask]
    wv = weights[mask]
    in_range = (bv >= 0) & (bv <= beta_grid[nbeta - 1])
    bv = bv[in_range]
    wv = wv[in_range]
    if len(bv) == 0:
        return
    indices = np.searchsorted(beta_grid[:nbeta], bv) - 1
    indices = np.clip(indices, 0, nbeta - 2)
    db = beta_grid[indices + 1] - beta_grid[indices]
    safe_db = np.where(db > 0, db, 1.0)
    frac = np.where(db > 0, (bv - beta_grid[indices]) / safe_db, 0.0)
    np.add.at(output, indices, wv * (1.0 - frac))
    np.add.at(output, np.minimum(indices + 1, nbeta - 1), wv * frac)


def _match_phonopy_atoms(ph, crystal_info):
    """Match phonopy primitive cell atoms to crystal_info atom types.

    Returns list of length natom_phonopy, each entry is the index into
    crystal_info['atom_types'], or -1 if unmatched.
    Uses element symbol matching (Mg→Z=12, O→Z=8, etc.).
    """
    from phonopy.interface.calculator import get_default_physical_units
    pcell = ph.primitive
    symbols = pcell.symbols
    # Map symbol → Z
    _symbol_to_Z = {
...
def _load_phonopy_for_coh_inel(phonopy_path, born_path=None):
    """Load phonopy object from phonopy_disp.yaml with force constants.

    Looks for FORCE_SETS and optionally BORN in the same directory.
    Returns the phonopy object ready for dynamical matrix calculations.
    """
    import phonopy as _phonopy
    import os
    base_dir = os.path.dirname(phonopy_path)
    force_sets = os.path.join(base_dir, "FORCE_SETS")
    if not os.path.exists(force_sets):
        raise FileNotFoundError(f"FORCE_SETS not found in {base_dir}")

    kwargs = dict(
        force_sets_filename=force_sets,
        produce_fc=True,
    )
    if born_path is None:
        born_candidate = os.path.join(base_dir, "BORN")
        if os.path.exists(born_candidate):
            born_path = born_candidate
    if born_path is not None:
        kwargs['born_filename'] = born_path

    ph = _phonopy.load(phonopy_path, **kwargs)
    if ph.force_constants is None:
        raise RuntimeError("Failed to produce force constants from phonopy data")
    return ph


def _compute_eigvec_dos_mesh(ph, atom_map, principal_idx, ni, delta_e, mesh=None):
    """Compute partial DOS for principal scatterer from phonopy BZ mesh.

    Directly histograms |e_d|² from mesh eigenvectors onto the LEAPR energy
    grid. This avoids Gaussian smearing artifacts and ensures exact grid
    alignment with the input DOS.

    Returns rho_eigvec on the same energy grid as the LEAPR input DOS,
    normalized per atom so that ∫ρ(ε)dε ≈ 3 (3 DOF per atom).
    """
    if mesh is None:
        mesh = [30, 30, 30]
    pcell = ph.primitive
    natom_ph = len(pcell.masses)
    n_principal = sum(1 for d in range(natom_ph) if atom_map[d] == principal_idx)
    if n_principal == 0:
        raise ValueError("No principal-type atoms found in phonopy primitive cell")

    # Run mesh with eigenvectors
    ph.run_mesh(mesh,
                with_eigenvectors=True, is_mesh_symmetry=False)
    mesh_dict = ph.get_mesh_dict()
    frequencies = mesh_dict['frequencies']        # (nqpt, nmodes) in THz
    eigenvectors = mesh_dict['eigenvectors']      # (nqpt, nmodes, natom*3)
    weights = mesh_dict.get('weights', None)       # (nqpt,)

    nqpt, nmodes = frequencies.shape

    # Build histogram on LEAPR energy grid
    energy_grid = np.arange(ni) * delta_e  # eV
    rho = np.zeros(ni)

    for iq in range(nqpt):
        w = weights[iq] if weights is not None else 1.0
        for nu in range(nmodes):
            freq_thz = frequencies[iq, nu]
            if freq_thz <= 0:
                continue
            energy_ev = freq_thz * THZ_TO_EV

            # Sum |e_d|² over principal-type atoms
            evec = eigenvectors[iq, :, nu]  # (natom*3,) complex
            weight_d = 0.0
            for d in range(natom_ph):
                if atom_map[d] == principal_idx:
                    e_d = evec[3*d:3*(d+1)]
                    weight_d += np.sum(np.abs(e_d)**2)
            weight_d /= n_principal  # per atom

            # Splat onto energy grid (linear interpolation)
            idx = energy_ev / delta_e
            i0 = int(idx)
            if i0 < 0:
                continue
            frac = idx - i0
            if i0 < ni:
                rho[i0] += w * weight_d * (1.0 - frac)
            if i0 + 1 < ni:
                rho[i0 + 1] += w * weight_d * frac

    # Normalize: sum of weights over BZ = nqpt (for uniform mesh)
    total_w = np.sum(weights) if weights is not None else nqpt
    # rho is a histogram with bin width delta_e
    # Convert to density: rho(ε) = histogram / (total_w * delta_e)
    # But for eigenvectors, Σ_ν |e_d|² = 3 for each atom d
    # So ∫rho(ε)dε = Σ_{q,ν} w_q * weight_d / total_w = 3
    rho /= (total_w * delta_e)

    return rho


def _precompute_dos_tensor(ph, atom_map, principal_idx, ni, delta_e, mesh=None):
    """Compute 3x3 partial DOS tensor rho_{ij}(omega) for principal species.

    rho_{ij}[n] = (1/Nq) sum_{q,nu} Re(e*_{d,i} e_{d,j}) * splat(omega, bin_n) / n_d

    For cubic crystals: rho_{ij} = delta_{ij} * rho_iso/3.
    Trace = isotropic partial DOS (same as _compute_eigvec_dos_mesh output).

    Returns shape (3, 3, ni).
    """
    if mesh is None:
        mesh = [30, 30, 30]
    pcell = ph.primitive
    ...
    mesh_dict = ph.get_mesh_dict()
    frequencies = mesh_dict['frequencies']      # (nqpt, nmodes) THz
    eigenvectors = mesh_dict['eigenvectors']    # (nqpt, natom*3, nmodes)
    nqpt, nmodes = frequencies.shape
    dos_tensor = np.zeros((3, 3, ni))
    ...
    for iq in range(nqpt):
        for nu in range(nmodes):
            ...
            # Sum Re(e*_i e_j) over principal-type atoms
            tensor_contrib = np.zeros((3, 3))
            for d in range(natom_ph):
                if atom_map[d] == principal_idx:
                    e_d = evec[3 * d:3 * (d + 1)]
                    tensor_contrib += np.real(np.outer(np.conj(e_d), e_d))
            tensor_contrib /= n_principal

            # Splat onto energy grid
            idx = energy_ev / delta_e
            i0 = int(idx)
            if i0 < 0:
                continue
            frac = idx - i0
            if i0 < ni:
                dos_tensor[:, :, i0] += tensor_contrib * (1.0 - frac)
            if i0 + 1 < ni:
                dos_tensor[:, :, i0 + 1] += tensor_contrib * frac

    dos_tensor /= (nqpt * delta_e)
    return dos_tensor
...
def _compute_msd_tensor(dos_tensor, ni, delta_e, tev, awr_d):
    """Compute 3x3 MSD tensor B from DOS tensor.

    B_{ij} = (WL2EKIN / (awr_d * kT)) × f0_{ij}

    where f0_{ij} = integral of rho_{ij}(omega)/(2*omega) * coth(omega/(2*kT)) domega,
    computed using the same discretization as fsum(n=0) in start().

    For isotropic atoms: B = (msd/3) * I (identity matrix).
    Tr(B) = msd_val (total <u²>) from _update_species_msd().

    Returns: 3x3 numpy array (units: Å²)
    """
    deltab = delta_e / tev  # beta spacing
    tau = 0.5
    arg = deltab * tau / 2.0
    edsq = exp(arg)

    # First normalize each component of the DOS tensor the same way as start().
    # start() normalizes by an = fsum(1, p, npt, tau, deltab) / tbeta
    # where tbeta=1.0 for partial spectra.
    # We need to normalize rho_{ij} consistently: the trace should integrate
    # to 3.0 (3 DOF). The normalization factor comes from the trace.

    # Compute normalization from the trace (isotropic DOS)
    rho_trace = dos_tensor[0, 0] + dos_tensor[1, 1] + dos_tensor[2, 2]

    # Transform trace to p(beta) format (same as start())
    p_trace = np.copy(rho_trace[:ni])
    p_trace[0] = p_trace[1] / deltab**2 if ni > 1 else 0.0
    v = exp(deltab / 2.0)
    u = deltab
    vv = v
    for j in range(1, ni):
        p_trace[j] = p_trace[j] / (u * (vv - 1.0 / vv))
        vv = v * vv
        u += deltab

    # Normalizing constant (fsum with n=1 for trace, tbeta=1)
    an = fsum(1, p_trace, ni, tau, deltab)

    # Now compute f0_{ij} for each tensor component
    f0_tensor = np.zeros((3, 3))
    for i in range(3):
        for j in range(3):
            rho_ij = dos_tensor[i, j, :ni]
            # Transform to p(beta) format
            p_ij = np.copy(rho_ij)
            p_ij[0] = p_ij[1] / deltab**2 if ni > 1 else 0.0
            u = deltab
            vv = v
            for k in range(1, ni):
                p_ij[k] = p_ij[k] / (u * (vv - 1.0 / vv))
                vv = v * vv
                u += deltab
            # Normalize by same factor as trace
            if abs(an) > 1e-30:
                p_ij /= an
            # f0 = fsum(n=0): integral of 2*p*cosh(tau*beta) dbeta
            f0_tensor[i, j] = fsum(0, p_ij, ni, tau, deltab)

    # B = (WL2EKIN / (awr_d * kT)) * f0_tensor
    B = f0_tensor * WL2EKIN / (awr_d * tev)
    return B


def _precompute_all_dos_tensors(ph, atom_map, n_species, ni, delta_e, mesh=None):
    """Compute DOS tensors for ALL species from a single mesh run.
    ...
    for d in range(natom_ph):
        sp = atom_map[d]
        if sp >= 0 and sp in tensors:
            species_atoms.setdefault(sp, []).append(d)

    for iq in range(nqpt):
        # Vectorize over modes: filter valid frequencies
        valid = frequencies[iq] > 0
        if not np.any(valid):
            continue
        valid_idx = np.where(valid)[0]
        energies = frequencies[iq, valid_idx] * THZ_TO_EV
        idx_e = energies / delta_e
        i0 = np.floor(idx_e).astype(int)
        frac = idx_e - i0

        # Eigenvectors for valid modes: (3*natom, nv) → (natom, 3, nv)
        evecs = eigenvectors[iq, :, valid_idx]
        evecs_3d = evecs.reshape(natom_ph, 3, -1)

        for sp_idx, atom_list in species_atoms.items():
            n_sp = n_per_species[sp_idx]
            e_sp = evecs_3d[atom_list]  # (n_sp_atoms, 3, nv)
            # Sum of outer products: contrib_{ij}(nu) = Re(sum_d conj(e_di) * e_dj) / n_sp
            contrib = np.real(np.einsum('adv,aev->dev', np.conj(e_sp), e_sp)) / n_sp
...
def _update_species_msd(crystal_info, itemp, tev, awr):
    """Compute mean-square displacement for each species at this temperature.

    Uses the partial phonon spectra from crystal_info. MSD in Å².
    Formula: <u²> = (ℏ²/2M) × ∫ ρ(ω)/ω × coth(ℏω/2kT) dω
    In our units with ρ(β) normalized: <u²> = WL2EKIN/(awr_d*tev) × f0_d
    where f0_d is the DW lambda from start() for species d.
    """
    for at in crystal_info['atom_types']:
        key = (at['Z'], at['A'])
        sp_idx = at.get('spectrum_idx')
        if sp_idx is None:
            continue
        sp = crystal_info['partial_spectra'][sp_idx]
        rho = np.array(sp['rho'][:sp['ni']])
        delta_e = sp['delta']  # eV
        ni = sp['ni']

        # Use start() to get f0 (DW lambda) for this species
        # tbeta for this species is always 1.0 (normalization of partial DOS)
        tbeta_sp = 1.0
        _, f0_sp, _, _ = start(rho, ni, delta_e, tev, tbeta_sp)

        # From contin: 2W = α*f0, α = Q²*WL2EKIN/(awr*kT)
        # So Q²<u²> = α*f0 → <u²> = f0 * WL2EKIN / (awr_d * kT)
        msd_val = f0_sp * WL2EKIN / (at['awr'] * tev)

        if key not in crystal_info['species_msd']:
            crystal_info['species_msd'][key] = [0.0] * (itemp + 1)
        while len(crystal_info['species_msd'][key]) <= itemp:
            crystal_info['species_msd'][key].append(0.0)
        crystal_info['species_msd'][key][itemp] = msd_val
...
def _derive_spectra_from_phonopy(crystal_info, mesh=None, sigma_thz=0.0,
    ...
    the principal species spectrum as the main DOS (replacing Cards 11/12).
    Optionally applies Gaussian smearing (sigma in THz).

    Stores in crystal_info:
      - 'partial_spectra': per-species partial DOS
      - 'phonopy_p1': main DOS for principal scatterer (replaces Card 12)
      - 'phonopy_delta1': energy spacing (replaces Card 11)
      - 'phonopy_np1': number of grid points (replaces Card 11)
      - 'all_dos_tensors': pre-computed DOS tensors for reuse
    """
    if mesh is None:
        mesh = [30, 30, 30]
    ph = crystal_info['_phonopy']
    atom_map = crystal_info['_atom_map']
    atom_types = crystal_info['atom_types']
    n_species = len(atom_types)
    principal_idx = crystal_info.get('principal_atom_idx', 0)

    # Determine energy grid from max frequency
    ph.run_mesh(mesh, with_eigenvectors=True, is_mesh_symmetry=False)
    md = ph.get_mesh_dict()
    freq_max_thz = np.max(md['frequencies'])
    freq_max_ev = freq_max_thz * THZ_TO_EV

    ni = nbin
    delta_e = freq_max_ev / (ni - 20)  # leave ~20 bins padding

    # Compute DOS tensors for all species
    all_tensors = _precompute_all_dos_tensors(
        ph, atom_map, n_species, ni, delta_e, mesh)

    # Apply Gaussian smoothing if requested
    sigma_ev = sigma_thz * THZ_TO_EV
    sigma_bins = sigma_ev / delta_e if delta_e > 0 else 0.0
    if sigma_bins > 0:
    ...
    partial_spectra = []
    for sp_idx in range(n_species):
        if sp_idx not in all_tensors:
            continue
        dt = all_tensors[sp_idx]
        rho_trace = dt[0, 0] + dt[1, 1] + dt[2, 2]
    ...
    # Store principal species DOS as the main spectrum (replaces Cards 11/12)
    principal_at = atom_types[principal_idx]
    for sp in partial_spectra:
        if sp['Z'] == principal_at['Z'] and sp['A'] == principal_at['A']:
            crystal_info['phonopy_p1'] = sp['rho'].copy()
            crystal_info['phonopy_delta1'] = delta_e
            crystal_info['phonopy_np1'] = ni
            break

    print(f"    Derived phonon data from phonopy ({mesh[0]}x{mesh[1]}x{mesh[2]} mesh, "
          f"sigma={sigma_thz:.2f} THz, nbin={ni}, delta={delta_e:.6e} eV)")
    for sp in partial_spectra:
        integral = np.sum(sp['rho']) * sp['delta']
        print(f"      Z={sp['Z']}, A={sp['A']}: integral={integral:.4f}")


def compute_onephonon_eigvec(alpha, beta, nalpha, nbeta, lat, arat,
                             tev, temp, awr, spr, crystal_info, itemp,
                             ndir=500, mesh=None, seed=42,
                             f0_input=None, delta1=None, np1=None, tbeta=1.0,
                             rho_precomputed=None, msd_tensors=None):
    """Compute eigenvector-based one-phonon scattering (self + distinct).

      - Self: compute partial DOS from BZ mesh → start() → construct S₁
              using f0_input for the alpha-dependent envelope (consistent
              with contin's multi-phonon terms).
      - Distinct: per-alpha BZ sampling with coherent structure factors.

    Parameters:
      f0_input: DW lambda from contin (for alpha envelope consistency)
      delta1: energy spacing of input DOS (eV)
      np1: number of DOS points
      tbeta: normalization parameter for principal spectrum
      mesh: BZ mesh as [nx, ny, nz] (default [30,30,30])
      rho_precomputed: if provided, skip mesh computation and use this DOS
      msd_tensors: dict of species_idx → 3×3 MSD tensor (Å²). If provided,
                   use anisotropic DW: exp(-½ Q·B·Q) per atom. If None,
                   fall back to isotropic DW: exp(-½ Q² msd).

    Returns (ss1, sd1) where:
      ss1[nbeta, nalpha] — self one-phonon S(α,β) from eigenvectors
      sd1[nbeta, nalpha] — distinct one-phonon (coherent cross-term),
                           NOT yet weighted by σ_coh/σ_b
    """
    ph = crystal_info['_phonopy']
    atom_map = crystal_info['_atom_map']
    atom_types = crystal_info['atom_types']
    ...
    # ================================================================
    # SELF one-phonon: mesh-based DOS → start() → S₁
    # ================================================================
    if delta1 is not None and np1 is not None and f0_input is not None:
        # Compute eigvec partial DOS from BZ mesh (or use pre-computed)
        if rho_precomputed is not None:
            rho_eigvec = rho_precomputed
        else:
            rho_eigvec = _compute_eigvec_dos_mesh(
                ph, atom_map, principal_idx, np1, delta1, mesh)

        # Feed through start() to get t₁ and f₀
        ...
        # Construct S₁_self using eigvec spectral function + input DW envelope
        al_vec = alpha[:nalpha] * sc / arat
        betan = beta[:nbeta] * sc

        # t₁(β) from eigvec DOS (same format as contin's p_out)
        # betan = beta * sc = actual dimensionless β (same as contin uses)
        ...
        # Fallback: zero self (contin handles it)
        betan = beta[:nbeta] * (THERM / tev if lat == 1 else 1.0)
        al_vec = alpha[:nalpha] * sc / arat
        ss1 = np.zeros((nbeta, nalpha))

    # ================================================================
    # DISTINCT one-phonon: per-alpha BZ sampling with self-consistent
    # normalization. We compute BOTH F_self and F_distinct from the
    # same BZ sampling, then normalize the distinct by requiring
    # consistency with the DOS-based self one-phonon (ss1).
    # ================================================================
    # Reciprocal lattice (with 2pi factor)
    rec_lat = np.linalg.inv(pcell.cell).T * 2.0 * pi
    # Cartesian positions of atoms in primitive cell (Å)
    pos_cart = pcell.scaled_positions @ pcell.cell

    # Pre-extract scattering lengths and masses
    b_coh = np.zeros(natom_ph)
    masses_amu = pcell.masses.copy()
    for d in range(natom_ph):
        idx = atom_map[d]
        if idx >= 0:
            b_coh[d] = atom_types[idx]['b_coh']

    # Pre-compute MSD per atom (scalar, for isotropic fallback)
    msd = np.zeros(natom_ph)
    if 'species_msd' in crystal_info:
        species_msd = crystal_info['species_msd']
        for d in range(natom_ph):
            idx = atom_map[d]
            if idx >= 0:
                at = atom_types[idx]
                key = (at['Z'], at['A'])
                if key in species_msd and itemp < len(species_msd[key]):
                    msd[d] = species_msd[key][itemp]

    # Pre-compute per-atom MSD tensor (3x3, for anisotropic DW)
    use_aniso_dw = msd_tensors is not None
    B_per_atom = None
    if use_aniso_dw:
        B_per_atom = np.zeros((natom_ph, 3, 3))
        for d in range(natom_ph):
            idx = atom_map[d]
            if idx >= 0 and idx in msd_tensors:
                B_per_atom[d] = msd_tensors[idx]

    # Precompute arrays for vectorized BZ sampling
    inv_sqrt_m = 1.0 / np.sqrt(masses_amu)  # (natom,)
    inv_rec_lat = np.linalg.inv(rec_lat)     # for BZ folding

    # Powder directions for distinct part
    directions = _random_sphere_directions(ndir, seed)
    ...
    for ia in range(nalpha):
        ...
        for idir in range(ndir):
            khat = directions[idir]
            Qvec = Qmag * khat

            # Fold Q into first Brillouin zone
            q_red = inv_rec_lat @ Qvec
            q_red = q_red - np.round(q_red)
            if np.max(np.abs(q_red)) < 1e-8:
                q_red = np.array([1e-6, 1e-6, 1e-6])

            # Diagonalize dynamical matrix at q
            dm.run(q_red.tolist())
            dynmat = dm.dynamical_matrix
            eigenvalues, eigvecs = np.linalg.eigh(dynmat)

            # Vectorized over modes and atoms
            valid_mask = eigenvalues > 0
            if not np.any(valid_mask):
                continue
            valid_idx = np.where(valid_mask)[0]
            freqs_thz = np.sqrt(eigenvalues[valid_mask]) * PHONOPY_FREQ_FACTOR
            omegas_ev = freqs_thz * THZ_TO_EV
            betas_nu = omegas_ev / tev
            range_mask = (betas_nu > 1e-10) & (betas_nu < 500)
            if not np.any(range_mask):
                continue

            mode_idx = valid_idx[range_mask]
            betas_v = betas_nu[range_mask]
            omegas_v = omegas_ev[range_mask]
            bose_v = 1.0 / (np.exp(betas_v) - 1.0)
            nv = len(mode_idx)

            # Eigenvectors for valid modes: (natom, 3, nv)
            evecs_valid = eigvecs[:, mode_idx].reshape(natom_ph, 3, nv)
            # Mass-normalize: (natom, 3, nv)
            evecs_scaled = evecs_valid * inv_sqrt_m[:, np.newaxis, np.newaxis]

            # Q·(e_d/sqrt(m_d)) for all atoms and modes: (natom, nv)
            Qe = np.einsum('i,aid->ad', Qvec, evecs_scaled)

            # DW factors per atom (same for all modes): (natom,)
            if use_aniso_dw:
                dw_atoms = np.exp(-0.5 * np.einsum('i,dij,j->d',
                                                     Qvec, B_per_atom, Qvec))
            else:
                dw_atoms = np.exp(-0.5 * Qsq * msd)

            # Phase factors: (natom,) complex
            phases = np.exp(1j * (pos_cart @ Qvec))

            # Amplitudes: A_d(nu) = (b/sqrt(m)) * dw * phase * (Q·e/sqrt(m))
            prefactor = b_coh * dw_atoms * inv_sqrt_m * phases  # (natom,)
            A = prefactor[:, np.newaxis] * Qe  # (natom, nv)

            # Structure factors per mode
            A_sum = np.sum(A, axis=0)  # (nv,)
            F_total = np.abs(A_sum)**2  # (nv,)
            A_self_sq_v = np.sum(np.abs(A)**2, axis=0)  # (nv,)
            F_distinct = F_total - A_self_sq_v  # (nv,)

            # Kinematic weights
            kinematic = (bose_v + 1.0) / (2.0 * natom_ph * omegas_v)

            # Batch splat to beta grid
            _batch_splat(betan, betas_v, A_self_sq_v * kinematic,
    ...
    # Normalize distinct using self-consistency with DOS-based self S₁.
    # The ratio C = ss1 / raw_self_grid should be a constant (α-independent)
    # because both have the same α-dependence (Q² × exp(-2W)).
    # Use the total (summed over all α, β) for robust normalization.
    sum_ss1 = np.sum(ss1)
...
def compute_generalized_inelastic(alpha, beta, nalpha, nbeta, lat, arat, tev,
    ...
    Phase 4 additions:
      - DOS tensors computed for ALL species (shared mesh)
      - Per-species 3×3 MSD tensor B from DOS tensor
      - Anisotropic DW in BZ sampling: exp(-½ Q·B_d·Q)

    For cubic crystals, every direction gives the same result as standard
    contin(), and B_d = msd×I, so generalized ≡ standard contin.
    ...
    _, f0_iso, tbar_iso, deltab_iso = start(p1_input.copy(), np1, delta1, tev, tbeta)

    # Step 2: DOS tensors — reuse pre-computed from _derive_spectra_from_phonopy
    # if available, otherwise compute fresh
    if 'all_dos_tensors' in crystal_info:
        ...
        all_dos_tensors = _precompute_all_dos_tensors(
            ph, atom_map, n_species, np1, delta1, mesh)

    # Principal species DOS tensor
    dos_tensor = all_dos_tensors[principal_idx]
    rho_trace = dos_tensor[0, 0] + dos_tensor[1, 1] + dos_tensor[2, 2]
    integral_trace = np.sum(rho_trace) * delta1
    print(f"      Principal DOS tensor trace integral: {integral_trace:.4f} (expect ~3.0)")

    # Step 3: MSD tensors for all species (Phase 4)
    msd_tensors = {}
    for sp_idx in range(n_species):
        if sp_idx not in all_dos_tensors:
            continue
        at = atom_types[sp_idx]
        B = _compute_msd_tensor(all_dos_tensors[sp_idx], np1, delta1, tev, at['awr'])
        msd_tensors[sp_idx] = B
        # Report: Tr(B) = <u²> should match scalar msd
        # B_{ij} = <u_i u_j>, so Tr(B) = <u²> (total MSD)
        msd_scalar = crystal_info['species_msd'].get((at['Z'], at['A']), [0.0])
        msd_scalar_val = msd_scalar[itemp] if itemp < len(msd_scalar) else 0.0
        tr_B = np.trace(B)
        ratio = tr_B / msd_scalar_val if msd_scalar_val > 1e-30 else float('nan')
        print(f"      Species {sp_idx} ({at['Z']},{at['A']}): "
              f"Tr(B)={tr_B:.6e}, <u²>_scalar={msd_scalar_val:.6e}, ratio={ratio:.6f}")
        # Anisotropy: B_xx=B_yy=B_zz=<u²>/3 for cubic, anisotropic otherwise
        print(f"        B_xx={B[0,0]:.6e}, B_yy={B[1,1]:.6e}, B_zz={B[2,2]:.6e}")

    # Step 4: Directional phonon expansion for self (parallelized across CPUs)
    directions = _random_sphere_directions(ndir, seed)
    ...
    # Step 5: Distinct one-phonon via compute_onephonon_eigvec
    # Pass msd_tensors for anisotropic DW (Phase 4)
    _, sd1 = compute_onephonon_eigvec(
        alpha, beta, nalpha, nbeta, lat, arat,
        tev, temp, awr, spr, crystal_info, itemp,
        ndir=ndir, mesh=mesh, seed=seed,
        f0_input=f0_iso, delta1=delta1, np1=np1, tbeta=tbeta,
        rho_precomputed=rho_trace, msd_tensors=msd_tensors)
...
def contin(ssm_slice, alpha, beta, nalpha, nbeta, lat, arat, tev,
    ...
    Returns: f0 (Debye-Waller lambda), tbar (effective temp ratio), deltab
    """
    tiny = 1.0e-30
    explim = -250.0
...
def run_leapr(input_file, output_file):
    ...
    if iel == 10:
        ...
        if ncoh_inel == 1:
            ...
            coh_inel_sigma = fvals[4]       # Gaussian smearing width in THz
            coh_inel_nbin = int(fvals[5])   # DOS grid points (default 300)
            coh_inel_ncpu = int(fvals[6])   # CPUs for parallel (0 = all available)
            # Read phonopy path as a quoted string
            coh_inel_phonopy_path = reader.read_string().strip("'\"")
            # Optionally read BORN file path on next card
            next_tok = reader.peek()
            if (next_tok is not None and next_tok != CARD_END
                    and isinstance(next_tok, tuple) and next_tok[0] == 'string'):
                coh_inel_born_path = reader.read_string().strip("'\"")
            ncpu_str = str(coh_inel_ncpu) if coh_inel_ncpu > 0 else "all"
            print(f"  Coherent inelastic: ndir={coh_inel_ndir}, "
                  f"mesh={coh_inel_mesh}, sigma={coh_inel_sigma} THz, "
                  f"nbin={coh_inel_nbin}, ncpu={ncpu_str}, "
                  f"phonopy={coh_inel_phonopy_path}")
            if coh_inel_born_path:
                print(f"    BORN file: {coh_inel_born_path}")
    ...
    if ncoh_inel_active:
        ...
        crystal_info['_phonopy'] = ph
        crystal_info['_atom_map'] = _match_phonopy_atoms(ph, crystal_info)
        crystal_info['species_msd'] = {}
        print(f"    Phonopy: {len(ph.primitive.masses)} atoms, "
              f"symbols={ph.primitive.symbols}")
        print(f"    Atom map: {crystal_info['_atom_map']}")

        # Derive all phonon data from phonopy
        mesh_coh = crystal_info.get('coh_inel_mesh', [30, 30, 30])
        sigma_thz = crystal_info.get('coh_inel_sigma', 0.0)
        nbin_coh = crystal_info.get('coh_inel_nbin', 300)
        _derive_spectra_from_phonopy(crystal_info, mesh=mesh_coh,
                                     sigma_thz=sigma_thz, nbin=nbin_coh)

        # Set p1, delta1, np1 from phonopy (replaces Cards 11/12)
        p1 = crystal_info['phonopy_p1']
        delta1 = crystal_info['phonopy_delta1']
        np1 = crystal_info['phonopy_np1']
        print(f"    Principal DOS from phonopy: delta1={delta1:.6e}, np1={np1}")
        ...
        for itemp in range(ntempr):
            ...
            if ncoh_inel_val == 1:
                ...
                ssm_self, sd1, f0, tbar, deltab = compute_generalized_inelastic(
                    ...
                    ndir=ndir_coh, mesh=mesh_coh, seed=42)

                ssm[:, :, itemp] = ssm_self
                dwpix[itemp] = f0
                tempf[itemp] = tbar * tempr_arr[itemp]

                # Add weighted distinct
                sigma_b = spr * ((1.0 + awr) / awr)**2
                p_idx = crystal_info['principal_atom_idx']
                sigma_coh_principal = crystal_info['atom_types'][p_idx]['sigma_coh']
                weight_d = sigma_coh_principal / sigma_b
